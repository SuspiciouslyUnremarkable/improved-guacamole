...
SUMMARY_REPORT = []
SQLFLUFF_CONFIG = Path("sql_format_tool/scripts/.sqlfluff")  # Updated path to .sqlfluff
...
def should_skip_formatting(sql: str) -> bool:
    existing_version = get_existing_pass1_version(sql)
    if not existing_version:
        return False
    return version_to_tuple(existing_version) >= version_to_tuple(PASS1_VERSION)
...
def add_noqa_comments(sql: str, lint_results: dict, filepath: Path) -> str:
    ...
    for violation in lint_results[0].get("violations", []):
...
def pass1_format_sql_file(filepath: Path, mirror: bool):
    ...
    if should_skip_formatting(original_sql):
